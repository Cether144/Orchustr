...
#[derive(Deserialize)]
struct DiscordMessage { id: String }
...
impl MessageSender for DiscordSender {
    ...
    async fn send(&self, msg: Message) -> Result<SendResult, CommsError> {
        ...
        let parsed: DiscordMessage = resp.json().await.map_err(|e| CommsError::Transport(e.to_string()))?;
        Ok(SendResult { message_id: parsed.id, channel: Channel::Discord, status: "sent".into() })
    }
}
