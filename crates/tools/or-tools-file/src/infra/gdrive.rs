...
impl GoogleDriveStore {
    pub fn from_env() -> Result<Self, FileError> {
        Ok(Self { client: reqwest::Client::new(), access_token: load_credential(TOKEN_ENV)? })
    }

    #[must_use]
    pub fn with_token(client: reqwest::Client, access_token: impl Into<String>) -> Self {
        Self { client, access_token: access_token.into() }
    }
...
#[derive(Deserialize)]
struct DriveFile { id: String, #[allow(dead_code)] name: String, #[serde(default)] size: Option<String>, #[serde(rename = "mimeType", default)] mime_type: String }
#[derive(Deserialize)]
struct DriveList { files: Vec<DriveFile> }

#[async_trait]
impl FileStore for GoogleDriveStore {
    async fn read(&self, path: &str) -> Result<FileContent, FileError> {
        // `path` is a file ID for Drive
        let text_resp = self.client.get(format!("{}/{}", FILES_URL, path))
            .header("Authorization", self.auth())
            .query_pairs([("alt", "media")])
            .send().await.map_err(transport)?;
        let status = text_resp.status().as_u16();
        if status != 200 {
            return Err(FileError::Upstream { provider: PROVIDER.into(), status, body: text_resp.text().await.unwrap_or_default() });
        }
        let content = text_resp.text().await.map_err(|e| FileError::Transport(e.to_string()))?;
        let size_bytes = content.len() as u64;
        Ok(FileContent { path: path.into(), content, size_bytes })
    }
    ...
    async fn write(&self, path: &str, content: &str) -> Result<(), FileError> {
        ...
        let body = format!(
            ...
            boundary = boundary, meta = meta, content = content
        );
        let resp = self.client.post(format!("{}?uploadType=multipart", UPLOAD_URL))
            .header("Authorization", self.auth())
        ...
        if !(200..300).contains(&status) {
            return Err(FileError::Upstream { provider: PROVIDER.into(), status, body: resp.text().await.unwrap_or_default() });
        }
        Ok(())
    }

    async fn list(&self, _path: &str) -> Result<Vec<FileEntry>, FileError> {
        let resp = self.client.get(FILES_URL)
            .header("Authorization", self.auth())
            .send().await.map_err(transport)?;
        let status = resp.status();
        if !status.is_success() {
            return Err(FileError::Upstream { provider: PROVIDER.into(), status: status.as_u16(), body: resp.text().await.unwrap_or_default() });
        }
        let list: DriveList = resp.json().await.map_err(|e| FileError::Transport(e.to_string()))?;
        Ok(list.files.into_iter().map(|f| FileEntry {
            path: f.id,
    ...
    async fn delete(&self, path: &str) -> Result<(), FileError> {
        let resp = self.client.delete(format!("{}/{}", FILES_URL, path))
            .header("Authorization", self.auth())
            .send().await.map_err(transport)?;
        let status = resp.status().as_u16();
        if status != 204 {
