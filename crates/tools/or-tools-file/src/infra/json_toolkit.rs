...
impl DataSource for JsonToolkit {
    ...
    async fn fetch(&self, query: Value) -> Result<Value, FileError> {
        let data = query.get("data").ok_or_else(|| FileError::Json("missing `data`".into()))?;
        let path = query.get("path").and_then(|v| v.as_array())
            .ok_or_else(|| FileError::Json("missing `path` array".into()))?;

        let mut current = data;
        for segment in path {
            let key = segment.as_str().ok_or_else(|| FileError::Json("path segment must be a string".into()))?;
...
mod tests {
    ...
    async fn resolves_nested_path() {
        let tk = JsonToolkit;
        let val = tk.fetch(json!({ "data": { "a": { "b": 42 } }, "path": ["a", "b"] })).await.unwrap();
        assert_eq!(val, 42);
    }
    ...
    async fn returns_null_for_missing_key() {
        let tk = JsonToolkit;
        let val = tk.fetch(json!({ "data": {}, "path": ["missing"] })).await.unwrap();
