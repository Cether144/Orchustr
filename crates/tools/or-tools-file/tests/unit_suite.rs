...
use or_tools_file::{FileError, FileStore, FileContent, FileEntry};
...
impl FileStore for MemStore {
    async fn read(&self, path: &str) -> Result<FileContent, FileError> {
        ...
        let content = lock.get(path).cloned().ok_or_else(|| FileError::NotFound(path.into()))?;
        let size = content.len() as u64;
        Ok(FileContent { path: path.into(), content, size_bytes: size })
    }
    ...
    async fn list(&self, _path: &str) -> Result<Vec<FileEntry>, FileError> {
        Ok(self.data.lock().await.keys().map(|k| FileEntry { path: k.clone(), size_bytes: 0, is_dir: false, modified_at: None }).collect())
    }
    async fn delete(&self, path: &str) -> Result<(), FileError> {
        self.data.lock().await.remove(path).map(|_| ()).ok_or_else(|| FileError::NotFound(path.into()))
    }
}
...
async fn file_store_tool_read_op() {
    ...
    let out = tool.invoke(ToolInput::new("file", json!({ "op": "read", "path": "a.txt" }))).await.unwrap();
...
async fn file_store_tool_unknown_op() {
    ...
    let err = tool.invoke(ToolInput::new("file", json!({ "op": "bork" }))).await.unwrap_err();
...
async fn json_toolkit_resolves_path() {
    ...
    let tk = JsonToolkit;
    let val = tk.fetch(json!({ "data": { "x": { "y": 99 } }, "path": ["x", "y"] })).await.unwrap();
    assert_eq!(val, 99);
}
...
async fn json_toolkit_returns_null_for_missing() {
    use or_tools_file::infra::json_toolkit::JsonToolkit;
    use or_tools_file::domain::contracts::DataSource;
    let val = JsonToolkit.fetch(json!({ "data": {}, "path": ["nope"] })).await.unwrap();
