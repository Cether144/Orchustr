...
impl ClickUpTracker {
    ...
    pub fn with_config(
        ...
        Self { client, base_url: base_url.into(), api_key: api_key.into(), list_id: list_id.into() }
    }
}

#[derive(Deserialize)]
struct TaskList { tasks: Vec<CuTask> }
#[derive(Deserialize)]
struct CuTask {
    id: String,
    name: String,
    ...
    status: CuStatus,
    #[serde(default)] assignees: Vec<CuUser>,
    #[serde(default)] tags: Vec<CuTag>,
}
#[derive(Deserialize)]
struct CuStatus { status: String }
#[derive(Deserialize)]
struct CuUser { username: String }
#[derive(Deserialize)]
struct CuTag { name: String }
#[derive(Deserialize)]
struct CuCreated { id: String }
...
impl ProjectTracker for ClickUpTracker {
    ...
    async fn list_issues(&self, _query: Value) -> Result<Vec<Issue>, ProductivityError> {
        ...
        let resp = self.client.get(url).header("Authorization", &self.api_key).send().await.map_err(transport)?;
        ...
        let list: TaskList = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(list.tasks.into_iter().map(|t| Issue {
            id: t.id,
            title: t.name,
    ...
    async fn create_issue(&self, issue: Issue) -> Result<String, ProductivityError> {
        ...
        let resp = self.client.post(&url).header("Authorization", &self.api_key).json(&body).send().await.map_err(transport)?;
        let status = resp.status();
        if !status.is_success() {
            return Err(ProductivityError::Upstream { provider: PROVIDER.into(), status: status.as_u16(), body: resp.text().await.unwrap_or_default() });
        }
        let created: CuCreated = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(created.id)
    }
}
