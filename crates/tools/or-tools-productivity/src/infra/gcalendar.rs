...
#[derive(Deserialize)]
struct EventList { #[serde(default)] items: Vec<GEvent> }
#[derive(Deserialize)]
struct GEvent {
    id: String,
    ...
    #[serde(default)] attendees: Vec<GAttendee>,
}
#[derive(Deserialize)]
struct GDateTime { #[serde(rename = "dateTime", default)] date_time: String }
#[derive(Deserialize)]
struct GAttendee { email: String }
...
impl CalendarClient for GoogleCalendarClient {
    ...
    async fn list_events(&self, query: Value) -> Result<Vec<CalendarEvent>, ProductivityError> {
        ...
        let url = build_url(&format!("{}/events", self.base_url), &[("timeMin", time_min), ("timeMax", time_max), ("singleEvents", "true")])?;
        ...
        let list: EventList = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(list.items.into_iter().map(|e| CalendarEvent {
            id: e.id,
            title: e.summary,
