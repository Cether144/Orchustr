...
#[derive(Deserialize)]
struct MessageListResponse { #[serde(default)] messages: Vec<MessageRef> }
#[derive(Deserialize)]
struct MessageRef { id: String }
#[derive(Deserialize)]
struct MessagePayload { headers: Vec<Header> }
#[derive(Deserialize)]
struct Header { name: String, value: String }
#[derive(Deserialize)]
struct MessageDetail { id: String, payload: MessagePayload, #[serde(default)] snippet: String }

fn header_val(headers: &[Header], name: &str) -> String {
    headers.iter().find(|h| h.name.eq_ignore_ascii_case(name)).map(|h| h.value.clone()).unwrap_or_default()
}
...
impl EmailClient for GmailClient {
    ...
    async fn list(&self, query: Value) -> Result<Vec<Email>, ProductivityError> {
        ...
        let max = query.get("max_results").and_then(|v| v.as_u64()).unwrap_or(10).to_string();
        let url = super::shared::build_url(
            &format!("{}/messages", self.base_url),
            &[("q", q), ("maxResults", &max)],
        )?;
        let resp = self.client.get(url).header("Authorization", self.auth()).send().await.map_err(transport)?;
        ...
        let list: MessageListResponse = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        let mut emails = Vec::new();
        for r in list.messages.into_iter().take(5) {
            let url2 = format!("{}/messages/{}", self.base_url, r.id);
            let r2 = self.client.get(&url2).header("Authorization", self.auth()).send().await.map_err(transport)?;
            if !r2.status().is_success() { continue; }
    ...
    async fn send_email(&self, email: Email) -> Result<String, ProductivityError> {
        let raw = format!(
            "From: {}\r\nTo: {}\r\nSubject: {}\r\n\r\n{}",
            email.from, email.to.join(","), email.subject, email.body
        );
        let encoded = STANDARD.encode(raw.as_bytes());
        let url = format!("{}/messages/send", self.base_url);
        let body = json!({ "raw": encoded });
        let resp = self.client.post(&url).header("Authorization", self.auth()).json(&body).send().await.map_err(transport)?;
        let status = resp.status();
        if !status.is_success() {
            return Err(ProductivityError::Upstream { provider: PROVIDER.into(), status: status.as_u16(), body: resp.text().await.unwrap_or_default() });
        }
        let v: Value = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(v.get("id").and_then(|i| i.as_str()).unwrap_or_default().into())
    }
}
