...
impl JiraTracker {
    pub fn from_env() -> Result<Self, ProductivityError> {
        ...
        Ok(Self { client: reqwest::Client::new(), base_url, auth_header })
    }

    pub fn with_config(client: reqwest::Client, base_url: impl Into<String>, auth_header: impl Into<String>) -> Self {
        Self { client, base_url: base_url.into(), auth_header: auth_header.into() }
    }
}

#[derive(Deserialize)]
struct JiraSearch { issues: Vec<JiraIssue> }
#[derive(Deserialize)]
struct JiraIssue { id: String, fields: JiraFields }
#[derive(Deserialize)]
struct JiraFields {
    summary: String,
    #[serde(default)] description: Option<String>,
    status: JiraStatus,
    #[serde(default)] assignee: Option<JiraUser>,
    #[serde(default)] labels: Vec<String>,
}
#[derive(Deserialize)]
struct JiraStatus { name: String }
#[derive(Deserialize)]
struct JiraUser { #[serde(rename = "displayName")] display_name: String }
#[derive(Deserialize)]
struct JiraCreated { id: String }
...
impl ProjectTracker for JiraTracker {
    ...
    async fn list_issues(&self, query: Value) -> Result<Vec<Issue>, ProductivityError> {
        let jql = query.get("jql").and_then(|v| v.as_str()).unwrap_or("ORDER BY created DESC");
        let url = format!("{}/rest/api/3/search", self.base_url);
        let body = json!({ "jql": jql, "maxResults": 50 });
        let resp = self.client.post(&url).header("Authorization", &self.auth_header).json(&body).send().await.map_err(transport)?;
        ...
        let search: JiraSearch = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(search.issues.into_iter().map(|i| Issue {
            id: i.id,
    ...
    async fn create_issue(&self, issue: Issue) -> Result<String, ProductivityError> {
        ...
        let created: JiraCreated = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        Ok(created.id)
    }
}
