...
impl SlackMessenger {
    pub fn from_env() -> Result<Self, ProductivityError> {
        Ok(Self { client: reqwest::Client::new(), base_url: BASE_URL.into(), bot_token: load_credential(TOKEN_ENV)? })
    }

    pub fn with_config(client: reqwest::Client, base_url: impl Into<String>, bot_token: impl Into<String>) -> Self {
        Self { client, base_url: base_url.into(), bot_token: bot_token.into() }
    }

    fn auth(&self) -> String { format!("Bearer {}", self.bot_token) }
}

#[derive(Deserialize)]
struct PostMessageResponse { ok: bool, #[serde(default)] ts: String, #[serde(default)] error: String }

#[derive(Deserialize)]
struct SearchResponse {
    ok: bool,
    #[serde(default)] messages: SearchMessages,
}
#[derive(Deserialize, Default)]
struct SearchMessages { #[serde(default)] matches: Vec<SearchMatch> }
#[derive(Deserialize)]
struct SearchMatch { #[serde(default)] text: String, #[serde(default)] permalink: String }
...
impl TeamMessenger for SlackMessenger {
    ...
    async fn post(&self, channel: &str, text: &str) -> Result<String, ProductivityError> {
        ...
        let parsed: PostMessageResponse = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        if !parsed.ok {
            return Err(ProductivityError::Upstream { provider: PROVIDER.into(), status: 200, body: parsed.error });
        }
        Ok(parsed.ts)
    }
    ...
    async fn search_messages(&self, query: Value) -> Result<Vec<Page>, ProductivityError> {
        ...
        let parsed: SearchResponse = resp.json().await.map_err(|e| ProductivityError::Transport(e.to_string()))?;
        if !parsed.ok { return Ok(Vec::new()); }
        Ok(parsed.messages.matches.into_iter().map(|m| Page {
