...
impl MilvusClient {
    ...
    #[must_use]
    pub fn with_config(client: reqwest::Client, base_url: impl Into<String>, token: Option<String>) -> Self {
        Self { client, base_url: base_url.into(), token }
    }

    fn req(&self, method: reqwest::Method, path: &str) -> reqwest::RequestBuilder {
        let url = format!("{}/v2/vectordb{}", self.base_url.trim_end_matches('/'), path);
...
impl VectorStoreClient for MilvusClient {
    ...
    async fn ensure_collection(&self, cfg: CollectionConfig) -> Result<(), VectorError> {
        ...
        let resp = self.req(reqwest::Method::POST, "/collections/create")
    ...
    async fn upsert(&self, batch: UpsertBatch) -> Result<(), VectorError> {
        let data: Vec<Value> = batch.items.into_iter().map(|item| json!({
            "id": item.id,
            "vector": item.vector,
            "metadata": item.metadata,
        })).collect();
        let body = json!({ "collectionName": batch.collection, "data": data });
        let resp = self.req(reqwest::Method::POST, "/entities/upsert")
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        expect_ok(PROVIDER, resp).await
    }
    ...
    async fn delete(&self, req: DeleteRequest) -> Result<(), VectorError> {
        ...
        let resp = self.req(reqwest::Method::POST, "/entities/delete")
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        expect_ok(PROVIDER, resp).await
    }
    ...
    async fn query(&self, filter: QueryFilter) -> Result<Vec<VectorMatch>, VectorError> {
        ...
        let resp = self.req(reqwest::Method::POST, "/entities/search")
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        let parsed: MilvusQueryResult = decode(PROVIDER, resp).await?;
        Ok(parsed.data.into_iter().map(|m| VectorMatch {
