...
use crate::domain::entities::{CollectionConfig, DeleteRequest, Distance, QueryFilter, UpsertBatch, VectorMatch};
...
impl VectorStoreClient for QdrantClient {
    ...
    async fn ensure_collection(&self, cfg: CollectionConfig) -> Result<(), VectorError> {
        ...
        let resp = self.req(reqwest::Method::PUT, &format!("/collections/{}", cfg.name))
    ...
    async fn upsert(&self, batch: UpsertBatch) -> Result<(), VectorError> {
        let points: Vec<Value> = batch.items.into_iter().map(|item| json!({
            "id": item.id,
            "vector": item.vector,
            "payload": item.metadata,
        })).collect();
        let body = json!({ "points": points });
        let resp = self.req(reqwest::Method::PUT, &format!("/collections/{}/points", batch.collection))
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        expect_ok(PROVIDER, resp).await
    }
    ...
    async fn delete(&self, req: DeleteRequest) -> Result<(), VectorError> {
        ...
        let resp = self.req(reqwest::Method::POST, &format!("/collections/{}/points/delete", req.collection))
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        expect_ok(PROVIDER, resp).await
    }
    ...
    async fn query(&self, filter: QueryFilter) -> Result<Vec<VectorMatch>, VectorError> {
        ...
        let resp = self.req(reqwest::Method::POST, &format!("/collections/{}/points/search", filter.collection))
            .json(&body).send().await.map_err(|e| transport(PROVIDER, e))?;
        let parsed: QdrantQueryResult = decode(PROVIDER, resp).await?;
        Ok(parsed.result.into_iter().map(|m| VectorMatch {
